...
public class GeofenceTransitionsIntentService extends IntentService {
    ...
    private void handleGeofenceEvent(ReactContext reactContext, GeofencingEvent geofencingEvent) {
        ...
        if (geofenceTransition == Geofence.GEOFENCE_TRANSITION_ENTER ||
            ...
            String eventName = "";
            if(geofenceTransition == Geofence.GEOFENCE_TRANSITION_ENTER) eventName = GeofenceModule.ENTER_GEOFENCE;
            if(geofenceTransition == Geofence.GEOFENCE_TRANSITION_EXIT) eventName = GeofenceModule.EXIT_GEOFENCE;
            for (Geofence geofence : triggeringGeofences) {
                WritableMap resultMap = Arguments.createMap();
                resultMap.putString("identifier", geofence.getRequestId());

                GeofencePackage.emitMessageToRN(reactContext, eventName, resultMap);
            }
        } else {
            // Log the error.
